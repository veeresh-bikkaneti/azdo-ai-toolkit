...
*   **Acceptance Criteria (Gherkin)**:
    *   **MANDATORY**: All ACs must be written in Gherkin syntax (`Feature` / `Scenario` / `Given-When-Then`).
    *   **Tooling**: Use the **`azdo-pbi-analyzer`** to auto-draft these scenarios from raw requirements.
    *   **Goal**: Ensure ACs are directly translatable to Automated Tests.
*   **Estimate**: Relative complexity (Story Points, T-Shirt).
*   **Vertical Slicing**: Break down Epics into small, independent, testable slices.
