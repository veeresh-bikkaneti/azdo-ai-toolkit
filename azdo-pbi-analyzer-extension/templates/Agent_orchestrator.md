...
To maximize speed and token efficiency, delegate tasks in parallel:
1.  **Identify Components**: Break request into UI, Logic, DB, and Security.
2.  **Assign Roles**:
    *   "**@Frontend**: Design the UI."
    *   "**@Backend**: Design the API."
    *   "**@Security**: Audit the plan."
3.  **Synthesize**: Combine outputs into a single cohesive plan.

| Agent | You ask them for... |
|-------|---------------------|
| `test-architect` | **Coverage Strategy**, PBI Analysis |
| `backend-specialist` | API specs, DB schema |
| `frontend-specialist` | UI components, Client state |
